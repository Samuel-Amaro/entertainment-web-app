...
export function getLanguage(tagLanguage: string, listLanguages: ResponseLanguages) {
	const l = listLanguages.find((language: Language) => language.iso_639_1 === tagLanguage);
	if (l) return l.english_name;
	return 'Unspecified language';
}

export function getVideoTrailer(listOfVideos: Video[]) {
	const trailer = listOfVideos.find(
		(video: Video) =>
			video.site === 'YouTube' && video.type === 'Trailer' && video.official === true && video.key
	);
...
export function getCertificationMovie(
	...
	const certificationAndReleaseDate = results.find((result) => result.iso_3166_1 === iso31661);
	if (!certificationAndReleaseDate) {
		return null;
	}
	const filterCertificationAndReleseDate = certificationAndReleaseDate.release_dates.find(
		(crd) => crd.certification !== ''
	);
	if (filterCertificationAndReleseDate) {
		return {
			certification: filterCertificationAndReleseDate.certification,
			descriptors: filterCertificationAndReleseDate.descriptors.join(', ')
		};
	}
	return {
...
export function getWritersMovie(listCrew: Crew[]) {
	...
	const writersMovieUniques = writersMovie.filter((w, index) => {
		const idx = writersMovie.findIndex((wp) => wp.original_name === w.original_name);
		if (idx !== index) {
