...
import { getContentRatingTvSerie, getLanguage, getVideoTrailer } from '@/utils';
...
export default async function Page({ params }: Props) {
	...
	const contentRating = getContentRatingTvSerie(contentRatingList.results, 'US');
	...
	return (
		<main className={styles.main}>
			<section className={styles.sectionSummary}>
				<div
					className={
						detailsTvSeries.poster_path ? styles.wrapperImagePoster : styles.wrapperImagePosterEmpty
					}
				>
				...
				<div>
					<h1 className={`headingL ${styles.title}`}>
						...
						<span>{`(${
							typeof new Date(detailsTvSeries.first_air_date).getFullYear() !== 'number' ||
					...
					<div className={styles.metadatas}>
						{contentRating && (
							<p className={styles.ratingContent} title={contentRating.descriptors}>
						...
						<ol className={styles.listCreators}>
							{detailsTvSeries.created_by.map((people) => (
								<li key={people.id} className={styles.cardCreator}>
									<span className={`headingS ${styles.namePeople}`}>{people.name}</span>
					...
					{detailsTvSeries.seasons.map((season) => {
						...
						return (
							<div key={season.id} className={styles.cardSeason}>
								<div className={season.poster_path ? undefined : styles.wrapperPosterSeasonEmpty}>
									{season.poster_path && (
										// eslint-disable-next-line @next/next/no-img-element
										<img
								...
								<div className={styles.cardContent}>
									<h3 className={`headingS ${styles.nameSeason}`}>{season.name}</h3>
									<h4 className={`headingXS ${styles.metadatasSeason}`}>
										{date.getFullYear()} | {season.episode_count} Episodes
									</h4>
									<p className={styles.presentation}>
										{season.name} of {detailsTvSeries.name} premiered on {monthTextLong}{' '}
