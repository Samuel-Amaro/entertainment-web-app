...
export default function Search({ searchFor, query, pageIndex }: PropsSearch) {
	...
	if (!data)
		throw new Error('Failed Fetch datas search to pagination, with error: ' + error?.message);

	return (
		<>
			<h2 className={`headingM ${styles.title}`}>Found {data.total_results} Results</h2>
			<Pagination
				pageIndex={pageIndex}
