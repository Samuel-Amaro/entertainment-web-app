...
export default function Navbar() {
	...
	const isActiveSegmentMovie = navLinksMovies.find((nav) => pathname.startsWith(nav.hrefText));
	const isActiveSegmentTvSeries = navLinksTvSeries.find((nav) => pathname.startsWith(nav.hrefText));

	return (
		<aside className={styles.navbar}>
			<div>
				{/*eslint-disable-next-line @next/next/no-img-element*/}
				<img src="/assets/logo.svg" alt="logo" width={33} height={27} className={styles.logo} />
			</div>
			<nav className={styles.nav}>
				{
					<>
						<Link href="/" title="Home" target="_self" rel="next" className={styles.linkWrapper}>
							<IconNavHome
								className={
