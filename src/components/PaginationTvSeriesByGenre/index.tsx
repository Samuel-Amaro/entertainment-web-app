...
async function fetcher(url: string) {
	...
	if (!response.ok) {
		throw new Error('Failed to fetch datas by tv series from genre, datas pagination');
	}
...
export default function PaginationTvSeriesByGenre({ idGenre, pageIndex, nameGenre }: Props) {
	const { data, error, isLoading } = useSWR(`/api/tv/genre/${idGenre}?page=${pageIndex}`, fetcher);

	if (isLoading) {
		return <SkeletonPagination limitRenderingItems={20} />;
	}

	if (!data) throw new Error('Failed Fetch datas pagination tv from genre para view');
